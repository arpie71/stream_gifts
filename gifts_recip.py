...
DATA_FILENAME = 'data/recips_to_lookup.csv'
...
> If an entry contains multiple recipients that are part of a family (for example, First Family Children), type in FAMILY  
> If an entry contains multiple recipients that are not part of a family (for example, James Bond and Q), type in GROUP  
> If the Wikidata match is unclear (for example, A Department of Defense Employee, type in UNCLEAR  
...
def show_screen(current_row, remaining_indices,student):
    ...
    st.write("Original recipient information: " + str(current_row["recip_orig"]))
    st.write("Abbreviated recipient information: " + str(current_row["recip_name"]))
    #st.write(f"Current index: {st.session_state.idx + 1} out of {len(remaining_indices)}")
    search_term = quote_plus(str(current_row["recip_name"]))
...
def show_navigation(current_original_idx, remaining_indices, gifts_df):
    ...
    with col4:
        if st.button("Save", help="Write changes to file"):
            ...
            #gifts_df.to_csv("data/recips_to_lookup.csv", index=False)
            #st.success("Saved successfully.")
